# Оркестратор "Матрёшка" — полный цикл рецензирования

## Назначение
Выполнить полное рецензирование научной статьи по методологии "Матрёшка": от глобального до синтеза итоговой рецензии.

## Входные данные
- Полный текст статьи (предоставляется пользователем)

## Язык рецензии
- Определяется автоматически по языку статьи
- Рецензия выводится на том же языке, что и статья
- Поддерживаются: русский, английский, французский, немецкий, испанский, китайский и другие
- Если язык не распознаётся — используется английский

## Ссылки на промпты

| Промпт | Ссылка |
|--------|--------|
| Глобальный | https://github.com/Famimot/ai-peer-review-prompt/blob/main/prompt_ru.md?plain=1 |
| Заголовок | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/title_analyzer.md?plain=1 |
| Аннотация | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/abstract_analyzer.md?plain=1 |
| Введение | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/introduction_analyzer.md?plain=1 |
| Методология | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/methodology_analyzer.md?plain=1 |
| Результаты | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/results_analyzer.md?plain=1 |
| Заключение | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/conclusion_analyzer.md?plain=1 |
| Литература | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/references_analyzer.md?plain=1 |
| Синтез | https://raw.githubusercontent.com/Famimot/ai-peer-review-prompt/main/subprompts/synthesis_analyzer.md?plain=1 |

## Алгоритм выполнения

### Шаг 1. Глобальный анализ
1. Загрузить содержимое промпта по ссылке "Глобальный"
2. Выполнить анализ статьи

### Шаг 2. Пофрагментный анализ
Последовательно выполнить анализ по каждому узкому промпту в указанном порядке:
1. Заголовок
2. Аннотация
3. Введение
4. Методология
5. Результаты
6. Заключение
7. Литература

Для каждого:
- Загрузить содержимое промпта по соответствующей ссылке
- Выполнить анализ соответствующего фрагмента статьи

### Шаг 4. Синтез сводной рецензии
1. Загрузить содержимое промпта по ссылке "Синтез"
2. На основе всех накопленных результатов сформировать итоговую рецензию
3. Добавить обязательную фразу о соответствии ГОСТ

## Формат итогового вывода
Следует структуре из промпта "Синтез", включая:
- Краткое содержание
- Актуальность
- Научная новизна (с определением I/P-Novelty)
- Методологическая корректность
- Достоверность результатов
- Качество визуализации
- Этические аспекты
- Сводная оценка по критериям (таблица с весами)
- Итоговый балл и рекомендация
- Тип документа и категория (А/Б/В/Г)
- Рекомендация по уровню журнала (Q1–Q4)
- Основные замечания
- Рекомендации по доработке
- Обязательная фраза о ГОСТ

## Примечание
Оркестратор выполняет все этапы автоматически, последовательно, без запроса промежуточных результатов. Результаты каждого этапа сохраняются для использования на следующих этапах.
